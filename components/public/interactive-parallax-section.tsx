...
export function InteractiveParallaxSection() {
  ...
  return (
    <section className="relative w-full h-[700px] md:h-[950px] bg-[#2A1B14] overflow-hidden flex flex-col items-center justify-center border-y-8 border-[#DC6835] font-sans">
      ...
      >
        <div className="grid grid-cols-4 gap-32 transform scale-150">
      ...
      <motion.div 
        ...
        animate={{ 
          ...
          y: (mousePosition.y * -40) + 120, // Pushing it down from the text
      ...
      {/* Floating Accents */}
      <motion.div 
        animate={{ x: mousePosition.x * 150, y: mousePosition.y * 120, rotate: mousePosition.x * 20 }}
      ...
      <motion.div 
        animate={{ x: mousePosition.x * -200, y: mousePosition.y * -150, rotate: mousePosition.x * -30 }}
